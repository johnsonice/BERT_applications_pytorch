...
class EarlyStopping():
    ...
    def eval_loss(self, epoch, loss):
        ...
        if (self.current_loss - self.best_loss) < -self.min_delta:
            self.best_loss = loss
            self.wait = 1
            print("\nEarlyStop Check: Epoch{}, loss is still going donw. keep training\n".format(epoch))
        else:
            ...
            if self.wait >= self.patience:
                ...
                print('\nTerminated Training for Early Stopping at Epoch %04i' % 
                      (self.stopped_epoch))
            self.wait += 1
            print("\nEarlyStop Check: Epoch{}, loss condition not satisified. accumulate wait: {}/{}\n".format(epoch,self.wait,self.patience))
