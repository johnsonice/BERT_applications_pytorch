...
import sys
sys.path.append('..')
from utils import InputExample,InputFeatures,DataProcessor,convert_examples_to_features,accuracy,warmup_linear,EarlyStopping
...
def load_and_prepare_data(args,processor,label_list,tokenizer,
                          data_folder=args.data_dir,
                          data_file_name=args.test_data_file_name):
    
    eval_examples = processor.get_dev_examples(data_folder,data_file_name)
    eval_features = convert_examples_to_features(eval_examples, label_list, args.max_seq_length, tokenizer)
    
    logger.info(" Num develoment set examples = %d", len(eval_examples))    
...
def train_model(args,processor,train_examples,label_list,tokenizer,model,device,
                n_gpu,optimizer,num_train_steps,t_total,early_stoping_obj=None,
                eval_dev = False):
    global_step = 0
    if args.do_train:
        ## load training data 
        ...
        ## load dev data
        if eval_dev:
            eval_data = load_and_prepare_data(args,processor,
                                              label_list,
                                              tokenizer,
                                              data_folder=args.data_dir,
                                              data_file_name=args.dev_data_file_name)
            # Run prediction for full data
            eval_sampler = SequentialSampler(eval_data)
            eval_dataloader = DataLoader(eval_data, sampler=eval_sampler, batch_size=args.eval_batch_size)
        
        if early_stoping_obj:
            print('\nMonitor eval loss for early stoping....\n')
        ...
        for epoch_idx in trange(int(args.num_train_epochs), desc="Epoch"):
            ## for each epoch, if evaluat dev set, need to turn mode back to training
            if eval_dev:
                model.train()
            ...
            ## if use early stoping 
            if early_stoping_obj:
                ## get eval loss
                results = evel_model(args,
                                     processor,
                                     label_list,
                                     tokenizer,
                                     model,
                ...
                print('\nAt epoch: {}, development evaluation loss: {}\n'.format(epoch_idx,results['eval_loss']))
                early_stoping_obj.eval_loss(epoch_idx,results['eval_loss'])
                if early_stoping_obj.stop_training:
                    return model,tr_loss,nb_tr_steps,global_step
                
    return model,tr_loss,nb_tr_steps,global_step

## do evaluation if true
def evel_model(args,processor,label_list,tokenizer,model,device,optimizer,
               tr_loss=-1,nb_tr_steps=-1,global_step=-1,export_results=True,eval_dataloader=None):
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        
        ## if eval_data is provided, do not reload
        if eval_dataloader: 
            pass
        else:
